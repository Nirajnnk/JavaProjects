...
public class Currency {
    ...
    public static Double convert(Double amount, Double exchangeValue) {
        ...
        return Math.round(price * 100d) / 100d; // Rounding to 2 decimal places
    }
    ...
    public static void main(String[] args) {
        ...
        // Find source and target currency exchange rates
        Currency source = null;
        Currency target = null;

        for (Currency currency : currencies) {
            if (currency.getShortName().equals(sourceCurrency)) {
                source = currency;
            }
            if (currency.getShortName().equals(targetCurrency)) {
                target = currency;
            }
        }

        if (source != null && target != null) {
            Double exchangeValue = source.getExchangeValues().get(targetCurrency);
